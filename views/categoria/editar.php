            ...
            <?php if(isset($_SESSION['error_login'])) : ?>
             ...
             <!--CONTENIDO PRINCIPAL-->
               <div class="col-12 col-lg-9 caja-principal border">
                  ...
                  <i class="fas ico-registro mt-2 ml-2 float-left fa-edit"></i>
                  Edita la Categoría <?=$cat->nombre?>
              </h1>

              <div class="row mb-3 mt-1">
                  <div class="col">
                    <h2 class="titulo-3 text-center mt-2">Ingresa el nombre nuevo de la categoría</h2>
                     <form action="<?=base_url?>categoria/save&id=<?=$cat->id?>" method="POST">
                      <div class="form-group mt-3">
                       <label for="name"><i class="fas ico-form mr-2 fa-address-card"></i><strong>Nombre</strong></label>
                       <input type="text" name="nombre" class="form-control" placeholder="La categoría ahora se llamará..." value="<?=isset($cat) && is_object($cat) ? $cat->nombre : ''; ?>" required>
                      </div>

                     <button type="submit" class="btn mt-3 mb-2 boton-enviar btn-block">Guardar</button>
                  </div>
              </div>
